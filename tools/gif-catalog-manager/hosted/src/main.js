...
function cleanShortcode(name) {
  if (!name) return '';
  const clean = name.replace(/\.[^/.]+$/, '').split(/[-_]+/).filter(p => !/^\d+$/.test(p)).join('_');
  return clean.toLowerCase().replace(/[^a-z0-9_]/g, '').slice(0, 32);
}

function formatTitle(name) {
  const sc = cleanShortcode(name);
  if (!sc) return '';
  return sc.charAt(0).toUpperCase() + sc.slice(1).replace(/_/g, ' ');
}

function cleanMediaInput(raw) {
  if (!raw) return { url: '', shortcode: '', title: '' };
  let str = String(raw).trim();

  // 1. Nested Markdown image: [![lechemoi](https://cdn3.emoji.gg/emojis/162404-lechemoi.gif)](https://emoji.gg/emoji/162404-lechemoi)
  const nestedMd = str.match(/\[\s*!\[(.*?)\]\((https?:\/\/[^\s\)]+)\)\s*\]\((?:https?:\/\/[^\s\)]+)?\)/i);
  if (nestedMd) {
    return {
      url: nestedMd[2],
      shortcode: cleanShortcode(nestedMd[1]),
      title: formatTitle(nestedMd[1])
    };
  }

  // 2. Simple Markdown image: ![alt](https://...)
  const simpleMd = str.match(/!\[(.*?)\]\((https?:\/\/[^\s\)]+)\)/i);
  if (simpleMd) {
    return {
      url: simpleMd[2],
      shortcode: cleanShortcode(simpleMd[1]),
      title: formatTitle(simpleMd[1])
    };
  }

  // 3. HTML img tag: <img ... src="https://..." alt="...">
  const htmlImg = str.match(/<img\b[^>]*\bsrc=["'](https?:\/\/[^"'\s]+)["'][^>]*>/i);
  if (htmlImg) {
    const altMatch = htmlImg[0].match(/alt=["']([^"']+)["']/i);
    const alt = altMatch ? altMatch[1] : '';
    return {
      url: htmlImg[1],
      shortcode: cleanShortcode(alt),
      title: formatTitle(alt)
    };
  }

  // 4. Markdown link: [alt](https://...)
  const mdLink = str.match(/\[(.*?)\]\((https?:\/\/[^\s\)]+)\)/i);
  if (mdLink) {
    let url = mdLink[2];
    const egg = url.match(/emoji\.gg\/emoji\/([0-9]+-[a-z0-9_-]+)/i);
    if (egg) url = `https://cdn3.emoji.gg/emojis/${egg[1]}.gif`;
    return {
      url,
      shortcode: cleanShortcode(mdLink[1]),
      title: formatTitle(mdLink[1])
    };
  }

  // 5. emoji.gg page URL: https://emoji.gg/emoji/162404-lechemoi
  const eggPage = str.match(/https?:\/\/emoji\.gg\/emoji\/([0-9]+-([a-z0-9_-]+))\/?/i);
  if (eggPage) {
    return {
      url: `https://cdn3.emoji.gg/emojis/${eggPage[1]}.gif`,
      shortcode: cleanShortcode(eggPage[2]),
      title: formatTitle(eggPage[2])
    };
  }

  // 6. Direct or standalone URL
  const anyUrl = str.match(/https?:\/\/[^\s\)\]"'>]+/i);
  if (anyUrl) {
    let url = anyUrl[0];
    const egg = url.match(/emoji\.gg\/emoji\/([0-9]+-([a-z0-9_-]+))/i);
    if (egg) {
      return {
        url: `https://cdn3.emoji.gg/emojis/${egg[1]}.gif`,
        shortcode: cleanShortcode(egg[2]),
        title: formatTitle(egg[2])
      };
    }
    return { url, shortcode: '', title: '' };
  }

  return { url: str, shortcode: '', title: '' };
}
...
function handleEmojiUrlInput() {
  ...
  const raw = $('#emoji-url').value;
  const cleaned = cleanMediaInput(raw);
  if (cleaned.url && cleaned.url !== raw && (raw.includes('[') || raw.includes('<') || raw.includes('emoji.gg/emoji/'))) {
    $('#emoji-url').value = cleaned.url;
  }
  const sc = cleaned.shortcode || extractShortcodeFromUrl(cleaned.url || raw);
  if (sc && !$('#emoji-shortcode').dataset.edited) {
    $('#emoji-shortcode').value = sc;
    if (!$('#emoji-title').dataset.edited) {
      $('#emoji-title').value = cleaned.title || formatTitle(sc);
    }
  }
}

$('#emoji-url')?.addEventListener('input', handleEmojiUrlInput);
$('#emoji-url')?.addEventListener('paste', () => setTimeout(handleEmojiUrlInput, 20));
...
$('#emoji-form')?.addEventListener('submit', async (event) => {
  ...
  setEmojiStatus();
  try {
    const rawUrl = $('#emoji-url').value.trim();
    const cleaned = cleanMediaInput(rawUrl);
    const url = cleaned.url || rawUrl;
    if (url && url !== rawUrl) {
      $('#emoji-url').value = url;
    }
