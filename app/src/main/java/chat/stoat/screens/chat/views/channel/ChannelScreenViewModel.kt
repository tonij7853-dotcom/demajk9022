...
class ChannelScreenViewModel(
    ...
    fun sendPendingMessage() {
        ...
        viewModelScope.launch {
            ...
            takenAttachments.forEachIndexed { index, it ->
                try {
                    val filename = if (it.spoiler) "SPOILER_${it.filename}" else it.filename
                    ...
                    val autumnUrl = "${chat.stoat.core.model.data.STOAT_FILES}/attachments/$id/$filename"
                    val persistentCacheDir = File(chat.stoat.StoatApplication.instance.cacheDir, "sent_media").apply { mkdirs() }
                    val cachedCopy = File(persistentCacheDir, "sent_${System.currentTimeMillis()}_$filename").apply {
                        try { it.file.copyTo(this, overwrite = true) } catch (_: Exception) {}
                    }
                    if (cachedCopy.exists()) {
                        chat.stoat.internals.LocalMediaCache.recordSentMedia(autumnUrl, cachedCopy)
                    }
                } catch (e: Exception) {
    ...
    suspend fun ackMessage(messageId: String) {
        ...
        StoatAPI.unreads.markAsRead(chId, messageId, sync = true)
    }
    ...
    private fun hydrateIncomingMessage(message: MessageFrame, expectedChannelId: String) {
        ...
        if (messageId != null) {
            viewModelScope.launch {
                try {
                    StoatAPI.unreads.markAsRead(expectedChannelId, messageId, sync = true)
                } catch (e: CancellationException) {
                    throw e
                } catch (e: Exception) {
